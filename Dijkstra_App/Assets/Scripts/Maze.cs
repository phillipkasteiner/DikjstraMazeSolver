...
public class Maze : MonoBehaviour
{
    ...
    public GameObject wall, floor, sphere, runningInPlace;
    ...
    void CheckNodes()
    {
        ...
        {
            var canvas = allNodes[i].cellFloor.transform.GetComponentInChildren<TextMesh>();
    ...
    void CreateWallsFloors()
    {
        ...
        for (int i = 0; i <= ySize - 1; i++)
        {
            ...
            {
                myPos = new Vector3(initialPos.x + (j * wallLength), 0f, initialPos.z + (i * wallLength) - wallLength / 2);
                tempFloor = Instantiate(floor, new Vector3(0,0,0), Quaternion.identity) as GameObject;
                tempFloor.transform.parent = floorHolder.transform; // put each floor as a gameObject child in floorHolder
                tempFloor.transform.position = myPos;
            }
        }
